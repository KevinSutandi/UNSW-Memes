...
export function authRegisterV1(email, password, nameFirst, nameLast) {
    ...
    const emailfound = dataStore.authers.find(item => item.authemail === email) 
    ...
    const handlefound = dataStore.authers.find(item => item.handlestring === handlestring);
