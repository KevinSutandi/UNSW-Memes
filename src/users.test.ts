...
describe('userProfile iteration 2 testing', () => {
  ...
  test('userProfile run success', () => {
    expect(userProfile(user2.token, user2.authUserId)).toStrictEqual(USER_OBJECT);
  })
  ...
  test('setEmail email address is already being used by another user', () => {
    expect(setEmail(user.token, 'onlyfortestttt06@gmail.com')).toStrictEqual(ERROR);
  })
  ...
  test('setName invalid Token', () => {
    expect(setName("wrong token", "name first", "name last")).toStrictEqual(ERROR);
  })
  ...
  test('getAllUsers  run success', () => {
    expect(usersAll(user.token)).toBeInstanceOf(Array<Object>);
  })
